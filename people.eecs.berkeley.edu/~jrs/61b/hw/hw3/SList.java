...
public class SList {
   ...
   *  equals(), it removes duplicate nodes so that only one consecutive newNode
  ...
  public void squish() {
    ...
    // System.out.println("squish method " + head.item);
    if (head != null) {
      SListNode slow = head, fast = head.next;
      while (size >=2 && fast != null) {
   ...
   *  Make new SListNodes, but just newNode the references to the items.
   **/
  ...
  public void twin() {
    ...
    while (copy != null) {
      // add a copied node
      //  fast.next = slow.next;
      SListNode newNode = new SListNode (copy.item, copy.next);
      // newNode.next = copy.next;
      copy.next = newNode;
      size++;
      // 移动到下一个需要twin还未twin的node
      copy=newNode.next;
    }
    // head = copy; // ???
  }
